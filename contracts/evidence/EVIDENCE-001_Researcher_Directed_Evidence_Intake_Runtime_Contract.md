...
The core interface accepts a revision-bound search request and produces a search session, normalized results, and an operation/cost receipt independently of a named provider. Adapters MAY represent search providers, registries, repositories, or indexes, but MUST NOT own investigation state, Candidate Evidence, review, publication, graph state, provenance policy, or truth. Google may later be an adapter, never the domain owner.

Initial implementation MUST use deterministic offline fixtures. A live adapter requires separate review of cost, privacy, licensing, attribution, quotas, failures, credentials, source retention, and authorization. Results remain discovery references until deliberately captured.

## 15. Search session structure

A search session MUST support: `searchSessionId`, `operationId`, `investigationId`, `manifoldRevisionId`, `principalId` or session authority, provider/adapter identity and version, normalized `query`, selected node/edge/artifact/contradiction/Research Vector context, request time, completion/cancellation state, result count, capture count, error status, cost receipt, version, idempotency key, and explicit zero Manifold effect.

## 16. Search result structure

A result MUST support: stable result identity, session and provider identity, `query`, `resultRank`, `title`, `sourceUrl`, `canonicalizedUrl`, domain, provider-attributed `snippet`, attribution and retention restrictions, media type if known, retrieval/capture timestamps, selected context, capture disposition, provenance, content hash/object reference only when actually acquired, and stale status. Unknown values remain unknown.
...
Guests MAY perform free Web Discovery, URL capture, researcher notes, and session-local review where implemented. Guest work is explicitly volatile and MUST NOT be represented as durably saved. Guests cannot persist uploads, durable Candidate Evidence, Research Inbox publication, Candidate Knowledge, or governed revisions. The UI MUST warn before data loss and before persistence-dependent actions, offer account creation/adoption where governed, and never silently discard or claim persistence.
