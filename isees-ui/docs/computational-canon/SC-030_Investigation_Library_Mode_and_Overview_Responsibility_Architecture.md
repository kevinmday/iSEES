...
### 5.3 Iterative research-flow presentation

The OVERVIEW research-flow presentation SHALL describe an iterative, researcher-directed research cycle, not a mandatory sequence. As governed inputs evolve, the Investigation Manifold MAY be deterministically recomputed and MAY expand, reshape, or expose research vectors. The authoritative definitions and authority boundaries for the Investigation Manifold, Research Vector, and Research Inbox remain in SC-003, SC-025, and SC-027 respectively; this presentation does not redefine them.

Stage information tooltips are explanatory, non-operational orientation controls. Opening or closing one MUST NOT navigate, change browser history or workspace mode, select or activate an investigation, mutate investigation or Manifold state, initiate computation, publish to the Research Inbox, call an API, open a deep-dive surface, or emit an analytics mutation. Tooltip text and presentation MUST NOT establish truth, Canon status, evidentiary meaning, or publication authority. Mathematics exposes relationships and directions; the researcher retains authority over interpretation, acceptance, and meaning.

#### 5.3.1 Event-Space Evolution explanation

OVERVIEW SHALL present Event-Space Evolution as a diagrammatic explanation, not live investigation output, and SHALL visibly disclose that status. It is an explanatory presentation only: it consumes no active-investigation state, computes no graph, and provides no navigation, publication, admission, acceptance, promotion, or recomputation operation. Any stage-linked emphasis SHALL be local, transient, non-operational presentation state; it SHALL leave every explanatory layer visible, SHALL NOT follow tooltip pinned/open state, and SHALL NOT announce transient changes.

The explanation SHALL distinguish canonical/governed topology from candidate material by both geometry and visual treatment. Candidate Evidence and Candidate Knowledge are separate candidate domains. Noncanonical Research Vectors may expose investigative directions without establishing Manifold membership. Preservation in the Research Inbox does not promote material, establish truth, or add it to the Manifold. These statements apply the existing authorities of SC-003, SC-019, SC-025, and SC-027 and do not redefine their domain contracts.

Studio and `.author` drafting, saving, export, and Candidate Knowledge publication have no direct effect on the production Manifold. The diagram MUST NOT depict a direct `.author`-to-Manifold connection. It MAY explain only the conceptual governed lifecycle: Studio/`.author` to Candidate Knowledge, then an explicit governed acceptance/admission gate, then eligible investigation input, and only then conditional deterministic recomputation. The dashed conceptual path MUST terminate at the visibly separate acceptance gate before any eligible-input continuation. In accordance with Studio V1 canon, the current production Studio-to-investigation publication and acceptance connection remains fail-closed and unavailable; OVERVIEW MUST visibly disclose: **“Governed lifecycle — production connection not currently available.”**

Only explicit governed acceptance/admission of eligible knowledge could revise investigation inputs. A revised governed input set MAY trigger deterministic recomputation; recomputation is conditional and MAY expand, contract, reshape, or expose relationships, and does not guarantee that nodes are added. The researcher governs interpretation, admission, acceptance, and meaning.

Event-Space Evolution SHALL be exposed as one accessible explanatory image with a visible heading and not-live disclosure, an accessible name, and a complete description of the candidate/canonical and unavailable-production boundaries. Its internal marks MUST NOT be controls or independent tab stops. The inline diagram SHALL scale without fixed rendered dimensions or narrow-screen horizontal overflow and SHALL stack beneath the research sequence before either region becomes crowded. Emphasis MUST NOT require motion; any transition or animation MUST be disabled under reduced-motion preference. Rendering, pointer emphasis, focus emphasis, tooltip use, drafting, saving, export, and Candidate Knowledge publication MUST have no effect on navigation, investigation selection or activation, Research Bridge publication, Knowledge or Studio runtime state, graph computation, or the production Manifold.
