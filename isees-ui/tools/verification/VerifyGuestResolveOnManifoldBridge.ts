...
assert.equal(candidate.knowledgeObject.relationships.some(relationship => relationship.targetId === target.identity.id), false, "explicit Canon relationship acceptance remains required");
...
assert.equal(candidate.knowledgeObject.relationships.some(relationship => relationship.targetId === target.identity.id), false);
