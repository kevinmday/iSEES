...
const studioShellPath = "src/author/components/StudioShell.tsx";
const studioInboxPath = "src/studio/components/StudioResearchInbox.tsx";
const typedResearchAdaptersPath = "src/studio/sources/TypedResearchSourceAdapters.ts";
...
const studioShell = readFileSync(studioShellPath, "utf8");
const studioInbox = readFileSync(studioInboxPath, "utf8");
const typedResearchAdapters = readFileSync(typedResearchAdaptersPath, "utf8");
...
for (const mode of ["MANIFOLD", "COMPARE", "LAYERS", "EVIDENCE"]) assert(inboxVisibility.includes(`WorkspaceMode.${mode}`), `shared Research Inbox visibility preserves ${mode}`);
includes(inboxVisibility, "WorkspaceMode.INTENTION", "Research Inbox visibility includes INTENTION in the existing allowlist");
for (const mode of ["OVERVIEW", "TIMELINE", "RESEARCH"]) assert(!inboxVisibility.includes(`WorkspaceMode.${mode}`), `shared Research Inbox visibility continues to exclude ${mode}`);
...
includes(studioShell, "<StudioResearchInbox />", "STUDIO composes its permanent Research Inbox");
includes(studioInbox, 'aria-label="STUDIO Research Inbox"', "STUDIO Research Inbox has a distinct accessible owner");
includes(studioInbox, 'data-permanent="true"', "STUDIO Research Inbox is structurally permanent");
...
assert((surface.match(/useState<string>\(\)/g) ?? []).length === 1 && surface.includes("const [feedback, setFeedback]"), "NarrativeWorkspace local state is limited to Research publication feedback"); pass("NarrativeWorkspace introduces no local comparison state");
...
assert(/narrative\.paragraphs\.map\(\(paragraph\) =>[\s\S]*?<p>\{paragraph\}<\/p>/.test(surface), "canonical narrative paragraphs render directly and in array order"); pass("canonical narrative paragraphs render directly and in array order");
includes(surface, "narrativePassageResearchAnchor", "narrative passage publication uses the canonical typed Research adapter");
includes(surface, "collectTypedResearchSource", "narrative passage publication uses the shared collection boundary");
includes(typedResearchAdapters, 'sourceWorkspace: "NARRATIVE"', "typed narrative publication preserves source workspace provenance");
includes(typedResearchAdapters, "sourceIdentity: context.passageId", "typed narrative publication preserves exact passage identity");
