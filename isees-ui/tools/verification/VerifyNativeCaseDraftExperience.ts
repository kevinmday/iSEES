...
for (const label of ["Working title", "Observation location", "Local observation date", "Local observation time", "Timezone", "Observer narrative (in their own words)", "Object shape", "Movement behavior", "Sound characteristics", "Lighting/visibility", "Observer context", "Witness count", "Environmental conditions", "Approximate duration in seconds", "Researcher notes", "Source/provenance statement", "Privacy classification", "Rights/publication restrictions"]) assert(formSource.includes(label), `missing field label ${label}`);
