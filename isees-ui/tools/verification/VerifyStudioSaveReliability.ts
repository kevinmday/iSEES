...
assert.match(inspector, /Artifact ID[\s\S]*Immutable revision[\s\S]*Dirty · unsaved changes[\s\S]*Studio V1 operation requires attention/, "canonical identity/revision and dirty state coexist with V1 operation failure");
...
assert.match(inspector, /projections do not include these unsaved changes/, "saved projections are clearly distinguished from the dirty draft");
