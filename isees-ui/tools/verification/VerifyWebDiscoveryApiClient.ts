...
const searchBody = { schemaVersion:"web-discovery-outcome/v1",searchSessionId:"opaque-session-1",operationId:"search-op-1",investigationId:"investigation-a",expectedInvestigationRevision:7,manifoldRevisionId:"investigation-aggregate:7",normalizedQuery:"query",queryNormalizationVersion:"web-discovery-query-normalization/v1",adapterId:"fixture",adapterVersion:"1",status:"COMPLETED",startedAt:timestamp,completedAt:timestamp,expiresAt:timestamp,resultCount:1,results:[result],providerAttribution:"Fixture",restrictions:[],warnings:[],error:null,receipt:{schemaVersion:"web-discovery-receipt/v1",operationId:"search-op-1",investigationId:"investigation-a",manifoldRevisionId:"investigation-aggregate:7",status:"COMPLETED",createdAt:timestamp,completedAt:timestamp,...zeroEffects} };
const captureBody = (idempotencyDisposition:"CREATED"|"REPLAYED") => ({schemaVersion:"web-discovery-capture-outcome/v1",operationId:"capture-op-1",searchSessionId:"opaque-session-1",resultId:"opaque-result-1",investigationId:"investigation-a",expectedInvestigationRevision:7,manifoldRevisionId:"investigation-aggregate:7",candidateId:"candidate-1",candidateRevision:0,origin:"DISCOVERY",intakePathway:"WEB_DISCOVERY",visibleOrigin:"WEB_DISCOVERED",lifecycleState:"DISCOVERED",acquisitionState:"NOT_REQUESTED",publicationState:"NOT_PUBLISHED",idempotencyDisposition,capturedAt:timestamp,source:{title:result.title,providerReturnedUrl:result.providerReturnedUrl,normalizedUrl:result.normalizedUrl,sourceDomain:result.displayDomain,snippet:result.snippet,mediaType:result.mediaType,attribution:result.attribution,retentionRestrictions:[],providerMetadata:result.providerMetadata},receipt:{schemaVersion:"web-discovery-capture-receipt/v1",operationId:"capture-op-1",searchSessionId:"opaque-session-1",resultId:"opaque-result-1",candidateId:"candidate-1",principalAuthority:"principal-a",investigationId:"investigation-a",expectedInvestigationRevision:7,manifoldRevisionId:"investigation-aggregate:7",providerAdapterId:"fixture",providerAdapterVersion:"1",providerResultId:"provider-1",normalizedQuery:"query",queryNormalizationVersion:"web-discovery-query-normalization/v1",resultRank:1,providerReturnedUrl:result.providerReturnedUrl,normalizedUrl:result.normalizedUrl,capturedAt:timestamp,idempotencyDisposition,...zeroEffects}});

const searchCommand = buildWebDiscoverySearchCommand({investigationId:"investigation-a",expectedInvestigationRevision:7,manifoldRevisionId:"investigation-aggregate:7",searchSessionId:"opaque-session-1",operationId:"search-op-1",query:"query",resultLimit:10,adapterId:"fixture",adapterVersion:"1",idempotencyKey:"search-key-1"});
const captureCommand = buildWebDiscoveryCaptureCommand({investigationId:"investigation-a",expectedInvestigationRevision:7,manifoldRevisionId:"investigation-aggregate:7",searchSessionId:"opaque-session-1",resultId:"opaque-result-1",operationId:"capture-op-1",idempotencyKey:"capture-key-1",researcherNote:"Human selected this result."});
...
deepEqual(Object.keys(serializedCapture).sort(),["expectedInvestigationRevision","idempotencyKey","investigationId","manifoldRevisionId","operationId","researcherConfirmation","researcherNote","resultId","schemaVersion","searchSessionId"].sort(),"capture allowlist");
