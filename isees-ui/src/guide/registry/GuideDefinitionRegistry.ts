...
import { isLayersSnapshot, resolveLayersGuideDefinition } from "./LayersGuideDefinitions.ts";
import { resolveOperationalGuideDefinition } from "./OperationalGuideDefinitions.ts";
...
export function resolveGuideDefinition(snapshot: Readonly<GuideContextSnapshot>): GuideResolution {
  try {
    const definition: GuideDefinition = isLayersSnapshot(snapshot)
      ...
      : resolveOperationalGuideDefinition(snapshot);
