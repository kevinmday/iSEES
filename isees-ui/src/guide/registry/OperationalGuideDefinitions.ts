...
const CONTENT: Readonly<Record<WorkspaceMode, Content>> = {
  ...
  [WorkspaceMode.LIBRARY]: {
    ...
    steps: ["Search or filter from the Library command panel.", "Select a System Canon item to open the mode-local Library Preview without changing active investigation identity.", "Open an active External Research card to read its verified official destination outside iSEES in a new tab; planned cards remain disabled.", "Treat Library imagery as contextual unless it is separately governed as evidence.", "Start or bring a new case, or explicitly open or resume qualified work.", "Keep empty saved investigations in LIBRARY and continue setup.", "Use MANIFOLD for deterministic relationship analysis after activation."],
