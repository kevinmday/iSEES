...
export function topologySimilaritySourceFromProjection(projection: LayersExperimentalPairProjection, labels?: Readonly<{ caseA: string; caseB: string }>): MetricIntelligenceSource {
  ...
  return immutable({ investigationId: required(projection.investigationId, "Investigation identity"), pairId: required(projection.pairId, "Pair identity"), value: contribution.similarity, availability: "AVAILABLE", evaluator: { key: contribution.evaluatorKey, version: contribution.evaluatorVersion }, normalization: { key: contribution.normalization.normalizationKey, version: contribution.normalization.normalizationVersion }, executionId: required(projection.executionId, "Execution identity"), inputProjectionId: required(input.inputProjectionId, "Input projection identity"), endpoints, canonicalConfiguredWeight: contribution.canonicalWeight!, participatingNormalizedWeight: contribution.participatingWeight, calculationTrace: contribution.calculationTrace, rawWeightedProduct: contribution.rawWeightedProduct, normalizedAggregateContribution: contribution.weightedContribution, classification: "EXPERIMENTAL / DERIVED / NON-CANONICAL" });
}
