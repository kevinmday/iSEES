...
import { resolveIntentionRenderDerivations, type IntentionProjectionRenderModel } from "./IntentionProjectionRenderModel";
...
export default function IntentionProjection3D({ model, width, height, cameraAction, onProjectedNodeSelect, onProjectedLinkSelect, onMetricDerivationSelect }: IntentionProjection3DProps) {
  ...
  useEffect(() => { const action=cameraAction?.action; if (!action) return; const graph=graphRef.current; if (!graph) return; if (action === "FIT" || action === "RESET") { fit(); return; } const camera=graph.camera(); const controls=graph.controls() as { target:{x:number;y:number;z:number;set?:(x:number,y:number,z:number)=>void}; update?:()=>void; minDistance:number; maxDistance:number }; const target={x:controls.target.x,y:controls.target.y,z:controls.target.z}; const position={x:camera.position.x,y:camera.position.y,z:camera.position.z}; if (action.startsWith("PAN_")) { const x=action==="PAN_LEFT"?-32:action==="PAN_RIGHT"?32:0; const y=action==="PAN_UP"?32:action==="PAN_DOWN"?-32:0; controls.target.set?.(target.x+x,target.y+y,target.z); controls.update?.(); graph.cameraPosition({x:position.x+x,y:position.y+y,z:position.z},{x:target.x+x,y:target.y+y,z:target.z},0); return; } const factor=action==="ZOOM_IN"?.8:1.25; graph.cameraPosition({x:target.x+(position.x-target.x)*factor,y:target.y+(position.y-target.y)*factor,z:target.z+(position.z-target.z)*factor},target,0); }, [cameraAction, fit]);
  return <div className="intention-canvas" data-projection-mode="3D" aria-label="INTENTION 3D presentation projection; depth and position are not scientific results">
    <ForceGraph3D ref={graphRef} width={Math.max(width,1)} height={Math.max(height,1)} graphData={graphData} backgroundColor="#050b16" showNavInfo={false} enableNodeDrag={false} cooldownTicks={0} onEngineStop={fit}
