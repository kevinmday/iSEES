...
import { intentionMetricExplanation, MetricIntelligenceTrigger } from "../../metric-intelligence";
...
function Derivation({ value, projectionId, onFeedback }: { readonly value: IntentionMetricDerivation; readonly projectionId?: string; readonly onFeedback: (message: string) => void }) {
  ...
  const metric = intentionMetricExplanation(value,`intention:derivation:${value.metricId}`);
  const research = useResearchBridge();
  return <><section><h3>{value.metricName} · {value.symbol}</h3><strong>{metric ? <MetricIntelligenceTrigger metric={metric}/> : value.availability === "AVAILABLE" ? `${value.displayValue ?? value.value}${value.units ? ` ${value.units}` : ""}` : value.availability}</strong><p>{value.availability} · {value.epistemicClassification}</p><p>Computational projection; not canonical Evidence.</p></section>
