import { IntentionAvailability, type IntentionAvailability as Availability } from "./IntentionAvailability.ts";
...
const entries = [
  ...
  descriptor({ id: IntentionEquationId.GOVERNING_MODEL, symbol: "UNAVAILABLE", name: "Complete INTENTION governing model unavailable", displayExpression: "INTENTION governing model = unavailable", description: "No repository-authoritative complete computable INTENTION model exists. M = g(L,T,S) belongs exclusively to event-space manifold construction and is not an INTENTION inference model.", inputs: [], availability: IntentionAvailability.THEORETICAL, reason: "No repository-authoritative complete computable INTENTION model exists." }),
