...
function section(sectionId:OperationalDossierSectionId,facts:readonly EntityDossierFact[],relationships:readonly EntityDossierRelationshipFact[],availability:readonly EntityDossierFieldAvailability[]):OperationalDossierSectionProjection{
 ...
 if(sectionId==="OPERATIONAL_SUMMARY")selected=facts.filter(f=>f.predicate==="documented_mission_roles");
...
export function projectOperationalDossierProfile(revision:EntityDossierRevision,graphNodeType:string):OperationalDossierProfileProjection{
 const resolved=resolveOperationalDossierProfile({canonicalEntityId:revision.entityIdentity.canonicalEntityId,governedEntitySubtype:revision.entityIdentity.entitySubtype,canonicalKnowledgeType:revision.entityIdentity.canonicalType,graphNodeType});
 ...
 return Object.freeze({profileId:resolved.profile.profileId,resolutionBasis:resolved.basis,sectionOrder:resolved.profile.sectionOrder,sections:Object.freeze(sections)});
}
