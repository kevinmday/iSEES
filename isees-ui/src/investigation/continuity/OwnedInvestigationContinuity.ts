...
import { createOperationalGraphFingerprint, rehydrateOperationalRevisionInvestigation } from "../revision/OperationalGraphRevision.ts";
...
type DurableRevision = Readonly<{ investigationId: string; revisionId: string; revisionNumber: number; parentRevisionId: string | null; graph: Readonly<Record<string, unknown>>; fingerprint: string; graphSchemaVersion: "investigation-operational-graph/v1"; algorithmVersion: string; actorAuthority: string; recordedAt: string; mutationKind: string; sourceIdentity: string | null }>;
interface Base { readonly activationSchemaVersion: typeof OWNED_ACTIVATION_SCHEMA_VERSION; readonly investigationId: string; readonly title: string; readonly objective: string | null; readonly lifecycle: "ACTIVE"; readonly createdAt: string; readonly modifiedAt: string; readonly version: number; readonly aggregateSchemaVersion: "investigation-aggregate/v1"; readonly aggregateRevision: number; readonly access: Readonly<{ kind: "RESEARCHER_OWNED" }>; readonly freshnessToken: string; readonly operationalRevisionHead: DurableRevision | null; readonly operationalRevisionLineage: readonly DurableRevision[]; }
...
export function parseOwnedActivationAggregate(value: unknown, expectedId: string): OwnedActivationAggregate {
  const item = rec(normalizeLegacyAuthority(normalizeLegacyAdoption(value))), access = rec(item?.access), op = rec(item?.operationalState);
  const keys = ["activationSchemaVersion", "investigationId", "title", "objective", "lifecycle", "createdAt", "modifiedAt", "version", "aggregateSchemaVersion", "aggregateState", "aggregateRevision", "access", "operationalState", "freshnessToken", "operationalRevisionHead", "operationalRevisionLineage"];
  ...
  if (item.aggregateState === "EMPTY") {
    if (!exact(op, ["kind", "workspaceId", "focusedEventId", "nodes", "edges"]) || op.kind !== "EMPTY" || op.workspaceId !== `workspace:${expectedId}` || op.focusedEventId !== null || !Array.isArray(op.nodes) || op.nodes.length !== 0 || !Array.isArray(op.edges) || op.edges.length !== 0 || item.aggregateRevision !== 0 || item.operationalRevisionHead !== null || !Array.isArray(item.operationalRevisionLineage) || item.operationalRevisionLineage.length !== 0) fail();
  } else if (item.aggregateState === "ADOPTED") { validateAdopted(op, item, expectedId); validateDurableRevisions(item, expectedId); } else fail();
  return Object.freeze(item) as unknown as OwnedActivationAggregate;
}

function normalizeLegacyAuthority(value: unknown): unknown {
  const item=rec(value), op=rec(item?.operationalState);
  if (!item || "operationalRevisionHead" in item || "operationalRevisionLineage" in item) return value;
  if (item.aggregateState==="EMPTY") return { ...item, operationalRevisionHead:null, operationalRevisionLineage:[] };
  const workspace=rec(op?.workspace), view=rec(op?.viewState);
  if (item.aggregateState!=="ADOPTED" || !workspace || !Array.isArray(workspace.nodes) || !Array.isArray(workspace.edges) || !view || !Array.isArray(view.activeLayers)) return value;
  const nodes=workspace.nodes.map(raw=>{const node=rec(raw)!;return {id:node.id,label:node.title,type:node.kind==="CANONICAL_EVENT"?"EVENT":node.kind==="QUESTION"?"HYPOTHESIS":"NARRATIVE",metadata:node.kind==="CANONICAL_EVENT"?{sourceId:node.canonicalEventId,canonicalEventId:node.canonicalEventId}:{adoptedKind:node.kind}};});
  const edges=workspace.edges.map(raw=>{const edge=rec(raw)!;return {id:edge.id,source:edge.sourceId,target:edge.targetId,relationship:edge.kind==="RELATED"?"ASSOCIATED_WITH":edge.kind,weight:1,rationale:["Preserved guest adoption."]};});
  const graph={nodes,edges,statistics:{nodeCount:nodes.length,edgeCount:edges.length,eventCount:nodes.filter(node=>node.type==="EVENT").length,facilityCount:0,artifactCount:0,personCount:0,organizationCount:0,locationCount:0,narrativeCount:nodes.filter(node=>node.type==="NARRATIVE").length,hypothesisCount:nodes.filter(node=>node.type==="HYPOTHESIS").length}};
  const source=rec(op?.source), recordedAt=(source?.snapshotUpdatedAt??source?.importedAt) as string;
  const revision={investigationId:item.investigationId,revisionId:"MIGRATION-BASELINE-0001",revisionNumber:1,parentRevisionId:null,graph,fingerprint:createOperationalGraphFingerprint(graph as never),graphSchemaVersion:"investigation-operational-graph/v1",algorithmVersion:"MIGRATION_BASELINE_WORKSPACE_V1",actorAuthority:"MIGRATION_AUTHORITY",recordedAt,mutationKind:"MIGRATION_BASELINE",sourceIdentity:(source?.guestInvestigationId??source?.eventId??null)};
  return {...item,operationalRevisionHead:revision,operationalRevisionLineage:[revision]};
}

function validateDurableRevisions(item: Readonly<Record<string, unknown>>, id: string): void {
  if (!Array.isArray(item.operationalRevisionLineage) || item.operationalRevisionLineage.length === 0) fail();
  let previous: string | null = null;
  for (let index=0; index<item.operationalRevisionLineage.length; index++) {
    const revision=rec(item.operationalRevisionLineage[index]); const graph=rec(revision?.graph);
    if (!revision || !exact(revision,["investigationId","revisionId","revisionNumber","parentRevisionId","graph","fingerprint","graphSchemaVersion","algorithmVersion","actorAuthority","recordedAt","mutationKind","sourceIdentity"]) || revision.investigationId!==id || !text(revision.revisionId) || revision.revisionNumber!==index+1 || revision.parentRevisionId!==previous || !graph || !Array.isArray(graph.nodes) || !Array.isArray(graph.edges) || revision.graphSchemaVersion!=="investigation-operational-graph/v1" || !text(revision.algorithmVersion) || !text(revision.actorAuthority) || !timestamp(revision.recordedAt) || !text(revision.mutationKind) || !(revision.sourceIdentity===null || text(revision.sourceIdentity))) fail();
    try { if (createOperationalGraphFingerprint(graph as never)!==revision.fingerprint) fail(); } catch { fail(); }
    previous=revision.revisionId as string;
  }
  const head=rec(item.operationalRevisionHead), tail=rec(item.operationalRevisionLineage[item.operationalRevisionLineage.length-1]);
  if (!head || !tail || head.revisionId!==tail.revisionId || JSON.stringify(head)!==JSON.stringify(tail)) fail();
}
...
export function materializeOwnedActivation(a: OwnedActivationAggregate): MaterializedOwnedActivation {
  ...
  const i = base(a, artifacts); i.workspace.imported_events = canonical.map(event_id => ({ event_id, source: "SYSTEM_CANON" })); i.workspace.focused_event_id = op.viewState.focusedEventId; i.workspace.active_layers = [...op.viewState.activeLayers]; i.workspace = Object.freeze(i.workspace);
  i.revisions = a.operationalRevisionLineage.map(value => ({ id:value.revisionId, revisionNumber:value.revisionNumber, timestamp:value.recordedAt, operator:value.actorAuthority, ...(value.parentRevisionId===null?{}:{parentRevisionId:value.parentRevisionId}), branch:"MAIN" as const, message:value.mutationKind, manifold:{ id:`operational:${encodeURIComponent(a.investigationId)}:${value.revisionId}`, timestamp:value.recordedAt, algorithmVersion:value.algorithmVersion, activeLayers:[...op.viewState.activeLayers] as never, graph:value.graph as never } })); i.currentRevisionId=a.operationalRevisionHead!.revisionId; const investigation = rehydrateOperationalRevisionInvestigation(i);
