...
function CorrespondenceColumn({ values }: { values: readonly TimelineCorrespondence[] }) {
  const timeline = useTimelineInspection();
  return <section className="timeline-correspondence"><h3>Potential correspondences</h3>{values.length === 0 ? <p>No comparable admitted records.</p> : values.map(value => <button type="button" key={value.correspondenceId} aria-pressed={timeline.inspection?.kind === "CORRESPONDENCE" && timeline.inspection.correspondence.correspondenceId === value.correspondenceId} onClick={() => timeline.inspect({ kind: "CORRESPONDENCE", investigationId: timeline.investigation!.id, focusedEventId: timeline.focusedEventId!, comparedEventId: timeline.effectiveCandidate!.comparisonEventId, correspondence: value })}><strong>{value.status === "UNRESOLVED" ? "Comparable · unresolved" : "Unavailable"}</strong><span>{value.explanation}</span></button>)}</section>;
}
