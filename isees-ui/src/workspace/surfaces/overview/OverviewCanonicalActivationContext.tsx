...
import { executeOverviewCanonicalActivationAndEnterWorkspace, type OverviewCanonicalActivationOutcome } from "./OverviewCanonicalActivationCommand";
...
interface OverviewCanonicalActivationValue { readonly canActivate: boolean; readonly canResume: boolean; readonly canImport: boolean; readonly importStatus: ActivationStatus; readonly importMessage:string|null; readonly destinationTitle:string|null; readonly status: ActivationStatus; readonly message: string | null; readonly activate: () => Promise<OverviewCanonicalActivationOutcome>; readonly resume: () => boolean; readonly importIntoActive:()=>Promise<void>; }
...
export function OverviewCanonicalActivationProvider({ children }: { readonly children: ReactNode }) {
  ...
  const canResume=alreadyActive&&!canImport&&(activeInvestigation?.revisions.length??0)>0;
  ...
  const activate = useCallback(() => {
    ...
    const promise = executeOverviewCanonicalActivationAndEnterWorkspace({
  ...
  const resume = useCallback(() => canResume && resumeCurrentInvestigation(runtime), [canResume, runtime]);
  ...
  const value: OverviewCanonicalActivationValue = Object.freeze({ canActivate, canResume, canImport, importStatus:currentImport?.status??"IDLE",importMessage:currentImport?.message??null,destinationTitle:activeInvestigation?.name??null,status: currentFeedback?.status ?? "IDLE", message: currentFeedback?.message ?? null, activate,resume,importIntoActive });
