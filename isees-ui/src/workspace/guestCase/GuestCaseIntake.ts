...
import {
  createGuestCandidateKnowledgeObject,
  createGuestCandidateKnowledgeObjects,
...
function suppliedTitle(content: NativeCaseDraftContent): string | null {
  return content.workingTitle.state === "SUPPLIED"
    ? content.workingTitle.value?.trim() || null
    : null;
}
...
export function createGuestCandidateInvestigation(
  ...
  if (validation.some(result => !result.valid)) {
    return Object.freeze({ status: "INVALID", validation });
  }
  const content = mapFormStateToContent(form);
  const title = suppliedTitle(content);
  if (title === null) {
    const guestValidation = validation.map(result => result.field === "workingTitle"
      ? Object.freeze({ ...result, valid: false, message: "Working title is required for a guest case." })
      : result);
  ...
  const adapterInput = {
    ...
    title,
    content,
    operatorId: identity.identity.operatorId,
    recordedAt,
  };
  const guestKnowledgeObjects = createGuestCandidateKnowledgeObjects(adapterInput);
  const knowledgeObject = createGuestCandidateKnowledgeObject(adapterInput);
  ...
  const revisionless: Investigation = Object.freeze({
    ...
    description: content.observationNarrative.state === "SUPPLIED"
      ? content.observationNarrative.value!
      : "Researcher-supplied temporary case.",
    createdAt: recordedAt,
    updatedAt: recordedAt,
    createdBy: identity.identity.operatorId,
    status: "DRAFT",
    workspace: Object.freeze({
      id: `workspace:${investigationId}`,
      name: candidate.title,
      ...
      focused_event_id: candidateId,
      guest_candidate_event: candidate,
      investigations: [],
      artifacts: [],
      active_layers: [],
...
export function submitGuestCase(
  form: NativeCaseDraftFormState,
  identity: OperatorIdentityState,
