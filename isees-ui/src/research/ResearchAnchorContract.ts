...
export function migrateResearchAnchor(value: ResearchAnchor | Record<string, unknown>): ResearchAnchor {
  ...
  if (legacy.experiment && typeof legacy.experiment === "object") {
    const experiment = legacy.experiment as { schemaVersion?: string; projection: { projectionId: string; executionId: string; provenance?: { experimentSchemaVersion?: string } } };
    const experimentSchemaVersion = experiment.schemaVersion === "layers-experiment/v2" && experiment.projection.provenance?.experimentSchemaVersion === "layers-experiment/v2" ? "layers-experiment/v2" : "layers-experiment/v1";
    // Historical captures stay v1 unless both explicit v2 discriminators are present.
    return { ...legacy, experiment: { ...(legacy.experiment as object), schemaVersion: experimentSchemaVersion }, schemaVersion: RESEARCH_ANCHOR_SCHEMA_VERSION, kind: "LAYERS_EXPERIMENT", sourceWorkspace: "LAYERS", sourceIdentity: `EXPERIMENT:${experiment.projection.projectionId}`, collectedAt, createdAt: collectedAt, classification: "RESEARCHER_GENERATED", sourceExecutionId: experiment.projection.executionId, sourceProjectionId: experiment.projection.projectionId, display: { title: `LAYERS experiment ${experiment.projection.projectionId}`, summary: "Researcher-generated experimental projection." }, insertability: { state: "INSERTABLE", reason: "Exact experiment execution and projection are available." }, capturedRepresentation: { schemaVersion: experimentSchemaVersion, mediaType: "application/json", value: legacy.experiment } } as ResearchAnchor;
  }
