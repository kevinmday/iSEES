...
export default function LayerCatalogMatrix({ selectedIds, baselineIds, restoreProfile, onSelectionChange }: Props) {
  ...
  return <div className="layer-catalog">
    <header className="layer-catalog__header">
      ...
      <dl className="layer-catalog__totals" aria-label="Catalog totals"><div><dt>Selected</dt><dd>{selected.length}</dd></div><div><dt>Operational</dt><dd>{layerCatalogTotals.operational}</dd></div><div><dt>Unavailable</dt><dd>{layerCatalogTotals.unavailable}</dd></div></dl>
    </header>
    <div className="layer-catalog__toolbar">
      <button type="button" onClick={() => onSelectionChange(operationalSelectableLayerIds)}>Select all operational</button>
      <button type="button" onClick={() => onSelectionChange([])} disabled={selected.length === 0}>Clear all</button>
      <button type="button" onClick={() => restoreProfile(CanonicalLayerProfileId.CANONICAL_BASELINE)}>Restore baseline</button>
    ...
    <div className="layer-catalog__families" aria-label="Layer catalog families">
      ...
      {families.map(group => <section className="layer-family" key={group.family.id} aria-labelledby={`layer-family-${group.family.id}`}>
        <header className="layer-family__header"><div><h4 id={`layer-family-${group.family.id}`}>{group.family.label}</h4><p>{group.family.description}</p><small>{group.members.length === CanonicalLayerCatalogFamilyTotal(group.family.id) ? `${group.members.length} members` : `${group.members.length} visible of ${CanonicalLayerCatalogFamilyTotal(group.family.id)}`} · {group.operationalCount} operational · {group.selectedOperationalCount} selected</small></div><div>
          <button type="button" aria-label={`Select ${group.family.label} family`} disabled={group.operationalCount === 0} onClick={() => onSelectionChange(selectCatalogFamily(selected, group.family.id))}>Select family</button>
        ...
        <div className="layer-family__grid">{group.members.map(layer => {
          ...
          return operational ? <button type="button" key={layer.id} className="layer-cell layer-cell--operational" aria-pressed={isSelected} onClick={() => onSelectionChange(isSelected ? selected.filter(id => id !== layer.id) : normalizeOperationalSelection([...selected, layer.id]))}>
            <span className="layer-cell__title"><strong>{layer.label}</strong><em>{isSelected ? "SELECTED" : "OPERATIONAL"}</em></span><code>{layer.id}</code><p>{layer.description}</p>{baseline && <small>CANONICAL BASELINE MEMBER</small>}
          </button> : <details className="layer-cell layer-cell--unavailable" key={layer.id}>
