...
import { LayersTopologyMetricValue } from "./LayersExperimentalIntelligence";
import { layersMetricExplanation, MetricIntelligenceTrigger } from "../../metric-intelligence";
  ...
  return <main className="layers-lab">
    ...
    {projection ? <>
      ...
      <section className="layers-lab__instrument">
        ...
        <dl className="layers-lab__metrics"><div><dt>Baseline</dt><dd>{projection.delta.baselineScore===undefined?"UNAVAILABLE":<MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"baseline",projection.delta.baselineScore,pct(projection.delta.baselineScore),"layers:baseline")}/>}</dd></div><div><dt>Experiment</dt><dd><LayersTopologyMetricValue projection={projection} caseA={pairView!.focusedEventId} caseB={pairView!.comparisonEventId}/></dd></div><div><dt>Signed delta</dt><dd>{projection.delta.scoreDelta === undefined ? "NOT MEANINGFUL" : <MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"delta",projection.delta.scoreDelta,`${projection.delta.scoreDelta>=0?"+":""}${(projection.delta.scoreDelta*100).toFixed(1)} pp`,"layers:delta")}/>}</dd></div></dl>
...
function Ledger({ projection, selected, inspect }: { projection: LayersExperimentalPairProjection; selected: import("./LayersPresentationSelection").LayersInspectionSelection; inspect: (value: import("./LayersPresentationSelection").LayersInspectionSelection) => void }) { return <section className="layers-lab__ledger"><Heading number="05" title="Contribution ledger"/><div className="layers-lab__table-wrap"><table><thead><tr>{["Layer ID","Classification","Mapping","Dimension","Availability","Similarity","Canonical weight","Participating weight","Weighted contribution / reason"].map(value=><th key={value}>{value}</th>)}</tr></thead><tbody>{projection.layerContributions.map(item => <tr key={item.layerId}><td><button type="button" aria-pressed={selected.kind === "CONTRIBUTION" && selected.layerId === item.layerId} onClick={() => inspect({kind:"CONTRIBUTION",layerId:item.layerId})}>{item.layerId}</button></td><td>{item.classification}</td><td>{item.operationalMappingStatus}</td><td>{item.canonicalDimension ?? "UNMAPPED"}</td><td>{item.availability}</td><td>{item.similarity===undefined?"UNAVAILABLE":<MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"similarity",item.similarity,pct(item.similarity),`layers:ledger:${item.layerId}:similarity`,item)}/>}</td><td>{item.canonicalWeight===undefined?"—":<MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"configuredWeight",item.canonicalWeight,String(item.canonicalWeight),`layers:ledger:${item.layerId}:configured-weight`,item)}/>}</td><td><MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"participatingWeight",item.participatingWeight,String(item.participatingWeight),`layers:ledger:${item.layerId}:participating-weight`,item)}/></td><td>{item.availability === "UNAVAILABLE" ? item.unavailableReason : <MetricIntelligenceTrigger metric={layersMetricExplanation(projection,"weightedContribution",item.weightedContribution,item.weightedContribution.toFixed(6),`layers:ledger:${item.layerId}:weighted-contribution`,item)}/>}</td></tr>)}</tbody></table></div></section>; }
