...
export interface StudioArtifactProjection {
  ...
  currentVersion: { versionId: string; versionNumber: number; authorSchemaVersion: string; document: unknown; sourceSnapshotIds: string[]; claims: Array<{ claimId: string; lineageState: "SUPPORTED" | "UNRESOLVED" | "CONTRADICTED" | "ORPHANED" }>; citations: Array<{ citationId: string; sourceSnapshotId: string }>; createdAt: string; comparisonContext?: { focusedEventId?: string; comparisonEventId?: string } };
  sourceSnapshots: Array<{ snapshotId: string; sourceInvestigationId: string; resolutionStatus: "AVAILABLE" | "STALE" | "MISSING" | "REDACTED" | "UNAVAILABLE" | "UNKNOWN" }>;
