...
export function restoreAuthorDocument(semantic:SemanticDocument,createdAt:string):ComputationalAuthorDocument{return {identity:{id:semantic.documentId,createdAt:new Date(createdAt)},metadata:{title:semantic.title,description:"Authoritative Studio V1 revision",author:"",modifiedAt:new Date(createdAt),version:1},type:"DOCUMENT",status:"SAVED",nodes:semantic.nodes.map(node=>{if(node.type==="HEADING")return{id:node.id,type:AuthorNodeTypes.HEADING,level:node.level,text:node.text};if(node.type==="CLAIM")return{id:node.id,type:AuthorNodeTypes.OBSERVATION,text:node.text,source:"AUTHOR",relatedReferences:[],createdAt:new Date(createdAt)};if(node.type==="PARAGRAPH"||node.type==="RESEARCHER_NOTE")return{id:node.id,type:AuthorNodeTypes.PARAGRAPH,text:node.text};throw new StudioV1AdaptationError(`Saved semantic node ${node.type} is read-only and cannot be opened in the current editor.`)})};}
