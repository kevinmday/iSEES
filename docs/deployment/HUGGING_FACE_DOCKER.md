...
I2 remains responsible for attaching and verifying persistent storage, selecting `/data/isees` paths, migration and recovery, and dependency-aware readiness. The governed persistent directory set is `databases`, `studio-outputs`, `candidate-evidence-blobs`, and `backups`; Candidate Evidence blob storage derives from `/data/isees/candidate-evidence-blobs` unless its explicit override is configured. Current Hugging Face documentation describes attached storage buckets as runtime volumes mounted at a selected path. The Python base and direct dependencies are pinned, but transitive Python dependencies are not hash-locked; completing a hash-locked dependency graph is a documented reproducibility gap.

Production authentication now verifies that `ISEES_PERSISTENT_ROOT` resides on a filesystem distinct from the container root. A directory merely baked into the image is classified as `EPHEMERAL`; a missing root is `ROOT_ABSENT`. Either condition makes `/ready` return 503 and makes every authentication repository dependency return `AUTHENTICATION_UNAVAILABLE` without creating a database. The authentication store itself must already be schema-compatible before production becomes ready. For the established bucket layout (`isees/databases/...`), mount the private bucket read-write at `/data`, not `/data/isees`, so the existing files resolve at `/data/isees/databases/...`.
