"""Explicit saved-revision Studio V1 PDF export orchestration."""
...
def _filename(title: str, revision_number: int, exported_at: str) -> str:
    slug = re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-")[:80] or "investigation-report"
    stamp = exported_at.replace("-", "").replace(":", "").split(".", 1)[0] + "Z"
    return f"isees-{slug}-r{revision_number:04d}-{stamp}.pdf"
...
class StudioV1ExportService:
    def __init__(self, store, output_store: StudioOutputStore, application_instance_id: str,
        ...
        self.worker_id, self.lease_duration = f"{application_instance_id}:pdf", lease_duration
    ...
    def create(self, owner: str, investigation: str, artifact: str, revision_id: str, *,
        ...
        if (format != "PDF" or template_version != TEMPLATE_VERSION or renderer_version != RENDERER_VERSION
                or configuration_hash != CONFIGURATION_HASH or not _HASH.fullmatch(configuration_hash)
                or not idempotency_key.strip() or len(idempotency_key) > 200):
            raise StudioV1Failure(FailureCode.EXPORT_CONFIGURATION_INVALID, "PDF export configuration is invalid.")
        ...
        try:
            ...
            key, output_hash, length = self.outputs.write(export_id, data)
            ...
            return self.store.update_export_state(export_id, "CURRENT", output_hash=output_hash,
                storage_key=key, media_type=MEDIA_TYPE, safe_filename=_filename(model.title, revision.revisionNumber, exported_at),
            ...
            message = exc.safe_message if isinstance(exc, RenderModelFailure) else (str(exc) if isinstance(exc, OutputStoreFailure) else "PDF rendering failed safely.")
